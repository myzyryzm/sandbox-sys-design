...
export function buildEndToEndRunPrompt({ systemId, processId, processName, durationSeconds, apiBase }) {
  return [
    ...
    `it lists client_list (stateless rows: requestsPerSecond — calls per second; rows for STATEFUL`,
    `clients (manifest node stateful:true): instances — concurrent session-loop instances to keep`,
    `alive), websocket_list (websocket client pools to keep connected — how many pool clients to`,
    `spawn + messages/s each), constraint_list (rules of the valid world you must UPHOLD — seed any`,
    `out-of-scope preconditions they imply and only use legal inputs) and failure_list (states that`,
    `mean the system is broken / poorly designed — probe for them).`,
    ``,
    ...
    `into the datastores with docker compose exec, keeping referential integrity). THEN drive`,
    `client_list for the duration (synthesizing legal arguments that reference the seeded ids,`,
    `chaining responses): call each stateless method at its requestsPerSecond, and for each stateful`,
    `row keep <instances> concurrent subprocess loops alive (per-instance LB_CLIENT_STATE store +`,
    `stable identity args; respawn on exit). If websocket_list is present, ALSO spawn each row's pool as a`,
    `subprocess for the whole run: node systems/${systemId}/ws-clients/<client>.mjs --count <clientCount>`,
    `--duration <run seconds> --rate <messagesPerSecond>, then parse its trailing __WS_RESULTS__ line`,
    `and fold spawned/connected/sent/delivered/duplicates/latency into the report (see the`,
    `sandbox-websocket skill). Probe the datastore/metrics/call log for the failure states,`,
