...
export function buildEndToEndRunPrompt({ systemId, processId, processName, durationSeconds, apiBase }) {
  return [
    ...
    `it lists client_list (methods + how often to call each, in seconds), websocket_list (websocket`,
    `client pools to keep connected — how many pool clients to spawn + messages/s each), constraint_list`,
    `(rules of the valid world you must UPHOLD — seed any out-of-scope preconditions they imply and only`,
    `use legal inputs) and failure_list (states that mean the system is broken / poorly designed — probe`,
    `for them).`,
    ``,
    ...
    `seeded ids, chaining responses). If websocket_list is present, ALSO spawn each row's pool as a`,
