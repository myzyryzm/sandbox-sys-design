...
const pct = (u: number | null | undefined) => (u == null ? '—' : `${Math.round(u * 100)}%`)

export default function ScalingTab({ systemId, node, onClose, onBusyChange }: EditTabProps) {
  const [state, setState] = useState<LlmWorkerState | null>(null) // this base's { scaler, policy } entry
  const [form, setForm] = useState<PolicyForm | null>(null) // policy form; seeded once from the registry
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [savedAt, setSavedAt] = useState(0)
  const [workerCount, setWorkerCount] = useState<number | string>(1 + (node.replicas?.instances?.length || 0))
