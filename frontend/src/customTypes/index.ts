...
import type { CustomRuntime, CustomTypeModule } from '../types/customTypes'
import consumerGroup from './consumerGroup/index'
import downloadCoordinator from './downloadCoordinator/index'
import llmWorker from './llmWorker/index'
import persistenceReader from './persistenceReader/index'

const MODULES: CustomTypeModule[] = [consumerGroup, downloadCoordinator, llmWorker, persistenceReader]

const TYPES: Record<string, CustomTypeModule> = {}
for (const m of MODULES) {
  for (const st of m.serviceTypes) TYPES[st] = m
}

export const CUSTOM_TYPES: Record<string, CustomTypeModule> = TYPES

// The module that renders a given node, or null for a plain (non-custom) node.
export function customTypeOf(node: ManifestNode | null | undefined): CustomTypeModule | null {
...
export const CUSTOM_RUNTIMES: CustomRuntime[] = [...new Set(MODULES)]
  .filter((m) => m.runtime)
  .map((m) => m.runtime!)
