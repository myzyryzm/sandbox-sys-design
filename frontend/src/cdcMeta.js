...
export const cdcOpsOf = (rule) => CDC_OPS.filter((op) => (rule?.operations || []).includes(op))
...
export const cdcRuleKey = (r) => `${r.table}|${r.stream}|${r.topic}`

// The trace-edge label for the rule's publish arrow (cdc worker → its stream), e.g.
// "publishes refund to refunds".
export const cdcEdgeLabel = (rule) => `publishes ${rule.table} to ${rule.topic}`
