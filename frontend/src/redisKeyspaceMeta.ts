...
import type { RedisKeyspace, RedisWriteMode } from './types/manifest'

// Row badge per declared type — ≤4 chars so it sits like the HTTP-verb badges.
export const REDIS_BADGE: Record<string, string> = {
  string: 'STR',
  list: 'LIST',
  set: 'SET',
  hash: 'HSET',
...
export const REDIS_WRITE_VERB: Record<string, string> = {
  string: 'SET', list: 'RPUSH', set: 'SADD', hash: 'HSET', zset: 'ZADD', stream: 'XADD', geo: 'GEOADD',
}
export const REDIS_READ_VERB: Record<string, string> = {
...
export const redisTypesCompatible = (declared?: string, observed?: string): boolean =>
...
export const keyspaceLabel = (ks: RedisKeyspace): string => ks.shorthand || ks.name
...
export const writeModeOf = (
  ks: RedisKeyspace | null | undefined,
  writerId?: string | null,
): RedisWriteMode | null => (writerId && ks?.writeModes?.[writerId]) || null
...
export const keyspaceEdgeLabel = (
  ks: RedisKeyspace,
